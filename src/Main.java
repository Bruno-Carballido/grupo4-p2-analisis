...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      while ((line = input.readLine()) != null) {// leo hasta salto de línea
        newLine = filaIncompleta.concat(line);
        fields = p.split(newLine, -1); // parseo campos por comas
  ...
  private static void search10MostActivePilots(Integer anio, Integer mes) {
    ...
    try {
      ...
      for (int i = 0; i < n - 1; i++) {
        ...
        if (i < 10)
          System.out.printf("%d -> %s: %s%n", i + 1, pListAux[n - i - 1], pilotosMentions.get(pListAux[n - i - 1]));
        else break;
      }
  ...
  private static void search15UsersMostTweets() {
    ...
    try {
      ...
      for (int i = 0; i < n - 1; i++) {
        ...
        if (i < 15) {
          System.out.printf("%d -> %s%n", i + 1, usuariosLst[n - i - 1]);
  ...
  public static void top7WithMoreFavourite() throws DatosIncorrectos {
    ...
    for (int i = 0; i < topUsers.length; i++) {
      System.out.printf("%d -> %s%n", i+1, topUsers[i].toString());
    }
